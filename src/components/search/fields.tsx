...
export function PassengerPicker({
  ...
  return (
    <div ref={ref} className="relative w-full">
      ...
      {open && (
        <div className="absolute top-full end-0 mt-2 w-64 bg-white rounded-xl border border-border shadow-card-hover p-4 z-40">
          <Counter
            title="بزرگسال"
            sub="۱ سال به بالا"
            value={adults}
...
export function CityPicker({
  ...
  return (
    <div ref={ref} className="relative w-full">
      ...
      />
      {open && (
        <ul className="absolute top-full inset-x-0 mt-2 bg-white rounded-xl border border-border shadow-card-hover overflow-hidden z-40 max-h-72 overflow-y-auto">
          {filtered.length === 0 ? (
