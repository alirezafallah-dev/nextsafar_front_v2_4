    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-hidden flex flex-col md:flex-row gap-3 p-3">
          ...
          {visible.map((e, idx) => {
            ...
            return (
              <div
                ...
                className={`relative flex gap-3 rounded-xl border p-3 transition ${
              ...
              >
                  ...
                  <a href={e.url} target="_blank" rel="noopener noreferrer" className="block">
                    {e.image ? (
                      ...
                      <img
                        src={e.image}
                        alt={e.title}
                        className="w-[130px] h-[130px] sm:w-[200px] sm:h-[200px] rounded-xl object-cover"
                      />
                    ) : (
                      <span className="w-[130px] h-[130px] sm:w-[200px] sm:h-[200px] rounded-xl bg-bg-sec flex items-center justify-center text-text-muted text-2xl">
                        {TYPE_FA[e.type]?.[0] ?? "•"}
                      </span>
                    )}
                  </a>
                ...
                <div className="flex-1 min-w-0 flex flex-col">
                  <div className="flex items-start gap-2">
                    ...
                    >
                      <h4 className="text-sm font-extrabold leading-6 group-hover:text-primary transition">
        ...
        <div className={`w-full md:w-1/3 relative ${tab === "list" ? "hidden md:block" : ""}`}>
          <div className="w-full h-full min-h-[320px] md:min-h-0 rounded-xl overflow-hidden border border-border shadow-sm bg-bg-sec">
            {withCoords.length > 0 ? (
