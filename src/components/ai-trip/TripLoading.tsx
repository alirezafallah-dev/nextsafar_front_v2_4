...
const POLL_TIMEOUT = 95000;   // هر درخواست حداکثر ۹۵ ثانیه (پردازش inline ممکن است طول بکشد)
const MAX_TRIES = 150;        // ≈ ۱۰ دقیقه سقف کل
...
export default function TripLoading({
  ...
  /* ✅FIX: refs برای callbackها تا deps فقط planId باشد */
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        setDebug(`plan #${planId} • HTTP ${res.status} • ${data?.status ?? "—"}`);
        ...
        /* pending یا خطای موقت → ادامه پولینگ */
      } catch {
        clearTimeout(to);
        setDebug(`plan #${planId} • network/timeout`);
      }
  ...
  }, [planId]); // ✅ فقط planId — با رندر والد ری‌استارت نمی‌شود
